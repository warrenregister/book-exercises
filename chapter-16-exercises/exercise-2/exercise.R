...
ggplot(diamonds) + geom_col(aes(x=cut, y=price, fill=clarity))


# Draw the same chart again, but with each element positioned to "fill" the y axis
ggplot(diamonds) + geom_col(aes(x=cut, y=price, fill=clarity)) +coord_flip()


# Draw the same chart again, but with each element positioned to "dodge" each other
ggplot(diamonds) + geom_col(aes(x=cut, y=price, fill=clarity), position = 'dodge')
...
# previous exercise).
ggplot(diamonds) + geom_point(aes(x=cut, y=clarity), position = 'jitter')
...
ggplot(diamonds) + geom_violin(aes(x=color, y=price)) + scale_y_log10()
# Log 10 scale makes points from 3000-10000 appear as far from eachother as points between
# 300 and 1000. It makes the y axis scale logarithmicallty, each step up increases by 10x.

# Another interesting plot: draw a plot of the diamonds price (y) by carat (x), 
# using a heatmap of 2d bins (geom_bin2d)
# What happens when you make the x and y channels scale logarithmically?
ggplot(diamonds) + geom_bin2d(aes(x=carat, y=price))
ggplot(diamonds) + geom_bin2d(aes(x=carat, y=price)) + scale_x_log10() + scale_y_log10()
# Heatmap becomes more linear when both axises are scaled logarithmically

# Draw a scatter plot for the diamonds price (y) by carat (x). Color each point
# by the clarity (Remember, this will take a while. Use a sample of the diamonds 
# for faster results)
sample = sample_n(diamonds, 1000)
ggplot(sample) + geom_point(aes(x=carat, y=price, color=clarity))

# Change the color of the previous plot using a ColorBrewer scale of your choice. 
# What looks nice?
ggplot(sample) + geom_point(aes(x=carat, y=price, color=clarity)) + scale_color_brewer(palette = 'Set3')
...
ggplot(diamonds) + geom_bar(aes(x=cut, fill=cut), width = 1) + coord_flip()
...
ggplot(sample) + geom_point(aes(x=price, y=carat, color=clarity)) + facet_wrap('color')
