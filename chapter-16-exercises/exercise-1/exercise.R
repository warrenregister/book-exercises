...
ggplot(diamonds_sample) + geom_point(aes(x=carat, y=price))

# Draw the same plot as above, but color each of the points based on their 
# clarity.
ggplot(diamonds_sample) + geom_point(aes(x=carat, y=price, color=clarity))
...
ggplot(diamonds_sample) + geom_point(aes(x=carat, y=price), color='blue')

# Draw a scatter plot for `diamonds_sample` of `price` by `carat`, where each
# point has an aesthetic _shape_ based on the diamond's `cut`.
ggplot(diamonds_sample) + geom_point(aes(x=carat, y=price, shape=cut))

# Draw a scatter plot for `diamonds_sample` of *`cut`* by `carat`, where each
# point has an aesthetic _size_ based on the diamond's *`price`*
ggplot(diamonds_sample) + geom_point(aes(x=cut, y=carat, size=price))


# Try coloring the above plot based on the diamond's price!
ggplot(diamonds_sample) + geom_point(aes(x=cut, y=carat, size=price, color=price))


# Draw a line plot (with line geometry) for `diamonds_sample`. The x-position 
# should be mapped to carat, y-position to price, and color to cut.
ggplot(diamonds_sample) + geom_line(aes(x=carat, y=price, color=cut))


# That's kind of messy. Try using `smooth` geometry instead.
ggplot(diamonds_sample) + geom_smooth(aes(x=carat, y=price, color=cut))
...
ggplot(diamonds_sample) + geom_col(aes(x=cut, y=price))


# Add an aesthetic property that will _fill_ each bar geometry based on the 
# `clarity` of the diamonds. 
# What kind of chart do you get?
ggplot(diamonds_sample) + geom_col(aes(x=cut, y=price, fill=clarity))
...
ggplot(diamonds_sample) + geom_point(aes(x=price, y=carat)) + geom_smooth(aes(x=price, y=carat, color=cut,alpha=.3))
